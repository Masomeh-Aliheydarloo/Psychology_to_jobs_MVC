...
export const ClassETestCom: React.FC<LoginProps> = ({ onLogin, showresult }) => {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      ...
      try {
        ...
        const response = await get("/api/classEtest");
  ...
  const selectQ = async (event: React.ChangeEvent<HTMLInputElement>, qlist: Question) => {
    ...
    setSQ(SQ.filter((q) => q.QEid !== qlist.QEid));

    let newQ: SelectedQuestion = { QEid: qlist.QEid, Score: parseInt(event.target.value), QEnumber: qlist.QEnumber };
  ...
  function score() {
    ...
    const list4 = SQ.filter((q) => q.QEnumber === 4);
    if (list4.length != 0) sum = sum + Number(list4[list4.length - 1].Score);

    console.log("list4", list4);
    console.log("sum", sum);

    const list5 = SQ.filter((q) => q.QEnumber === 5);
    if (list5.length != 0) {
    ...
    const list6 = SQ.filter((q) => q.QEnumber === 6);
    ...
    const list7 = SQ.filter((q) => q.QEnumber === 7);
    if (list7.length != 0) {
    ...
    const list8 = SQ.filter((q) => q.QEnumber === 8);
    if (list8.length != 0) sum = sum + Number(list8[list8.length - 1].Score);
    console.log("list8", list8);
    console.log("sum", sum);

    const list9 = SQ.filter((q) => q.QEnumber === 9);
    if (list9.length != 0) sum = sum + Number(list9[list9.length - 1].Score);
    console.log("list9", list9);
    console.log("sum", sum);

    const list10 = SQ.filter((q) => q.QEnumber === 10);
    if (list10.length != 0) sum = sum + Number(list10[list10.length - 1].Score);
    console.log("list10", list10);
    console.log("sum", sum);


    const list11 = SQ.filter((q) => q.QEnumber === 11);
    if (list11.length != 0) {
      if (list11[list11.length - 1].Score === 1) { sum = sum + 6 }
      else if (list11[list11.length - 1].Score === 2) { sum = sum + 5 }
      else if (list11[list11.length - 1].Score === 3) { sum = sum + 4 }
      else if (list11[list11.length - 1].Score === 4) { sum = sum + 3 }
      else if (list11[list11.length - 1].Score === 5) { sum = sum + 2 }
      else if (list11[list11.length - 1].Score === 6) { sum = sum + 1 }
    }
    console.log("list11", list11);
    console.log("sum", sum);

    const list12 = SQ.filter((q) => q.QEnumber === 12);
    if (list12.length != 0) sum = sum + Number(list12[list12.length - 1].Score);

    console.log("list12", list12);
    console.log("sum", sum);

    const list13 = SQ.filter((q) => q.QEnumber === 13);
    if (list13.length != 0) {
    ...
    const list14 = SQ.filter((q) => q.QEnumber === 14);
    if (list14.length != 0) sum = sum + Number(list14[list14.length - 1].Score);
    console.log("list14", list14);
    console.log("sum", sum);

    const list15 = SQ.filter((q) => q.QEnumber === 15);
    if (list15.length != 0) sum = sum + Number(list15[list15.length - 1].Score);

    console.log("list15", list15);
    console.log("sum", sum);

    const list16 = SQ.filter((q) => q.QEnumber === 16);
    if (list16.length != 0) sum = sum + Number(list16[list16.length - 1].Score);

    console.log("list16", list16);
    console.log("sum", sum);

    const list17 = SQ.filter((q) => q.QEnumber === 17);
    if (list17.length != 0) sum = sum + Number(list17[list17.length - 1].Score);

    console.log("list17", list17);
    console.log("sum", sum);

    const list18 = SQ.filter((q) => q.QEnumber === 18);
    if (list18.length != 0) sum = sum + Number(list18[list18.length - 1].Score);
    console.log("list18", list18);
    console.log("sum", sum);


    const list19 = SQ.filter((q) => q.QEnumber === 19);
    if (list19.length != 0) {
      if (list19[list19.length - 1].Score === 1) { sum = sum + 6 }
      else if (list19[list19.length - 1].Score === 2) { sum = sum + 5 }
      else if (list19[list19.length - 1].Score === 3) { sum = sum + 4 }
      else if (list19[list19.length - 1].Score === 4) { sum = sum + 3 }
      else if (list19[list19.length - 1].Score === 5) { sum = sum + 2 }
      else if (list19[list19.length - 1].Score === 6) { sum = sum + 1 }
    }
  ...
  return (<>
    <div className="pb-5 pt-5 pl-8 pr-8">
       <p className="  font-roboto  text-base font-semibold md:text-l  text-text-error">
        CLASS E: EMOTIONAL EXPERIENCE and EXPRESSION<br/></p>
    ...
    <div className="w-full max-w-screen-sm md:max-w-screen-md mx-auto px-4 " >
      ...
      <fieldset className="  text-center space-y-8 justify-center items-center ">
        {Q.map((qlist) => (
          ...
          <div key={qlist.QEid} className="  justify-center items-center    p-4 rounded-md hover:text-text_light  text-text_black dark:text-text_white shadow-sm transition duration-200">
            <legend className="font-roboto font-normal md:font-medium text-base mb-4" > {qlist.c}/23.{qlist.QEtitle}  </legend>
            <div className="flex flex-wrap justify-center items-center gap-5 md:gap-12 mt-4">



              <input type="radio" className="  cursor-pointer w-14 h-14  mr-4  " name={`question-${qlist.QEnumber}`} id={`disagree-q1-3-${qlist.QEid}`} value={1} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className=" cursor-pointer w-10 h-10 mr-4" name={`question-${qlist.QEnumber}`} id={`disagree-q1-2-${qlist.QEid}`} value={2} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-8 h-8 mr-4" name={`question-${qlist.QEnumber}`} id={`neutral-q1-${qlist.QEid}`} value={3} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-8 h-8 mr-4" name={`question-${qlist.QEnumber}`} id={`agree-q1-2-${qlist.QEid}`} value={4} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-10 h-10 mr-4" name={`question-${qlist.QEnumber}`} id={`agree-q1-3-${qlist.QEid}`} value={5} onChange={(event) => selectQ(event, qlist)} />

              <input type="radio" className="  cursor-pointer w-14 h-14  mr-4 " name={`question-${qlist.QEnumber}`} id={`disagree-q1-4-${qlist.QEid}`} value={6} onChange={(event) => selectQ(event, qlist)} />
