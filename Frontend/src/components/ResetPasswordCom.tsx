        ...
        try {
            ...
            if (response.data === 1) {
                ...
                setShowResultno(true);// Not updated
                 ...
                 setError({ password: true }); // Incorrect current password
            }
        ...
        <>
           ...
           dark:bg-dark_bg_body justify-center  ">
                ...
                <div className="  col-span-2  mt-10 mr-5 ml-10 justify-center mb-5 ">
                    <p className='font-roboto font-medium text-xl md:text-2xl mb-5'>Password has been reset. You can now log in.</p>
                    <form onSubmit={handleSubmit(onSubmit)} >


                        <label className="block pb-2 font-roboto text-sm   md:text-base   text-text_black dark:text-text_white" htmlFor="uname" >New Password*</label>

                         <div className="flex flex-row ">
                                                    <input className="ring-1 font-roboto text-sm   md:text-base  outline-none ring-gray-500 p-2 rounded-md w-3/4  " type={type}
                                                        required {...register("password")} onFocus={() => setError({ password: false })} />
                                                    &nbsp;&nbsp;&nbsp;{showpassword ? (
                        ...
                        <div >
                            <button type="submit" className="font-roboto font-medium text-base  
                                 text-text_white dark:text-text_black bg-blue-700 rounded-lg px-7 mt-3   py-2.5 me-2 mb-2 
                                  bg-light_bg_subnav dark:bg-dark_bg_subnav">Reset Password</button>
                        ...
                        {showResult && (
                             <label className="text-text-error mt-5 mb-10 font-roboto font-medium text-sm">
                               🎉  Congratulation, your new password has been reset.
                            </label>)}
                        {showResultno && (
                         <label className="text-text-error mt-5 mb-10 font-roboto font-medium text-sm">
                                 &#10060;Unfortunately, Failed to reset your password. Please try again.
