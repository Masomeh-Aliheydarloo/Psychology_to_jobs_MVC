...
export const ClassDTestCom: React.FC<LoginProps> = ({ onLogin, showresult }) => {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      ...
      try {
        ...
        const response = await get("/api/classDtest");
  ...
  const selectQ = async (event: React.ChangeEvent<HTMLInputElement>, qlist: Question) => {
    ...
    setSQ(SQ.filter((q) => q.QDid !== qlist.QDid));

    let newQ: SelectedQuestion = { QDid: qlist.QDid, Score: parseInt(event.target.value), QDnumber: qlist.QDnumber };
  ...
  function score() {
    ...
    const list1 = SQ.filter((q) => q.QDnumber === 1);
    if (list1.length != 0) sum = sum + Number(list1[list1.length - 1].Score);

    console.log("list1", list1);
    console.log("sum", sum);

    const list2 = SQ.filter((q) => q.QDnumber === 2);
    if (list2.length !== 0) {
    ...
    const list3 = SQ.filter((q) => q.QDnumber === 3);
    if (list3.length != 0) sum = sum + Number(list3[list3.length - 1].Score);

    console.log("list3", list3);
    console.log("sum", sum);

    const list4 = SQ.filter((q) => q.QDnumber === 4);
  if (list4.length !== 0) {
      if (list4[list4.length - 1].Score === 1) { sum = sum + 6 }
      else if (list4[list4.length - 1].Score === 2) { sum = sum + 5 }
      else if (list4[list4.length - 1].Score === 3) { sum = sum + 4 }
      else if (list4[list4.length - 1].Score === 4) { sum = sum + 3 }
      else if (list4[list4.length - 1].Score === 5) { sum = sum + 2 }
      else if (list4[list4.length - 1].Score === 6) { sum = sum + 1 }
    }

    console.log("list4", list4);
    console.log("sum", sum);

    const list5 = SQ.filter((q) => q.QDnumber === 5);
    ...
    const list6 = SQ.filter((q) => q.QDnumber === 6);
    ...
    const list9 = SQ.filter((q) => q.QDnumber === 9);
    ...
    const list10 = SQ.filter((q) => q.QDnumber === 10);
    ...
    const list14 = SQ.filter((q) => q.QDnumber === 14);
    ...
    const list15 = SQ.filter((q) => q.QDnumber === 15);
    if (list15.length != 0) {
    ...
    const list16 = SQ.filter((q) => q.QDnumber === 16);
    if (list16.length != 0) sum = sum + Number(list16[list16.length - 1].Score);

    console.log("list16", list16);
    console.log("sum", sum);

    const list17 = SQ.filter((q) => q.QDnumber === 17);
    if (list17.length != 0) {
      if (list17[list17.length - 1].Score === 1) { sum = sum + 6 }
      else if (list17[list17.length - 1].Score === 2) { sum = sum + 5 }
      else if (list17[list17.length - 1].Score === 3) { sum = sum + 4 }
      else if (list17[list17.length - 1].Score === 4) { sum = sum + 3 }
      else if (list17[list17.length - 1].Score === 5) { sum = sum + 2 }
      else if (list17[list17.length - 1].Score === 6) { sum = sum + 1 }
    }

    console.log("list17", list17);
    console.log("sum", sum);

    const list18 = SQ.filter((q) => q.QDnumber === 18);
    ...
    const list22 = SQ.filter((q) => q.QDnumber === 22);
    if (list22.length != 0) sum = sum + Number(list22[list22.length - 1].Score);

    console.log("list22", list22);
    console.log("sum", sum);

 const list23 = SQ.filter((q) => q.QDnumber === 23);
    if (list23.length != 0) {
      if (list23[list23.length - 1].Score === 1) { sum = sum + 6 }
      else if (list23[list23.length - 1].Score === 2) { sum = sum + 5 }
      else if (list23[list23.length - 1].Score === 3) { sum = sum + 4 }
      else if (list23[list23.length - 1].Score === 4) { sum = sum + 3 }
      else if (list23[list23.length - 1].Score === 5) { sum = sum + 2 }
      else if (list23[list23.length - 1].Score === 6) { sum = sum + 1 }
    }

    console.log("list23", list23);
    console.log("sum", sum);


     const list24 = SQ.filter((q) => q.QDnumber === 24);
    if (list24.length != 0) sum = sum + Number(list24[list24.length - 1].Score);
  ...
  return (<>
    <div className="pb-5 pt-5 pl-8 pr-8">
        ...
        CLASS D: DISCLOSURE and INTERPERSONAL CLOSENESS <br/>
        <p className="  font-roboto font-medium text-base  ">This questionnaire will ask you to respond to a number of statements. You are asked to read each statement
carefully, and then think about whether the statement applies to you or does not apply to you. Then circle the
number that best describes how much you agree with the statement.<br /></p>
     
       <p className="  font-roboto  text-base font-semibold md:text-l  text-text-error"></p>
    </div>
    ...
    <div className="w-full max-w-screen-sm md:max-w-screen-md mx-auto px-4 " >
      ...
      <fieldset className="  text-center space-y-8 justify-center items-center ">
        {Q.map((qlist) => (
          ...
          <div key={qlist.QDid} className="  justify-center items-center    p-4 rounded-md hover:text-text_light  text-text_black dark:text-text_white shadow-sm transition duration-200">
            <legend className="font-roboto font-normal md:font-medium text-base mb-4" > {qlist.c}/24.{qlist.QDtitle}  </legend>
             <div className="flex flex-wrap justify-center items-center gap-5 md:gap-12 mt-4">


              <input type="radio" className="  cursor-pointer w-14 h-14  mr-4  " name={`question-${qlist.QDnumber}`} id={`disagree-q1-3-${qlist.QDid}`} value={1} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className=" cursor-pointer w-10 h-10 mr-4" name={`question-${qlist.QDnumber}`} id={`disagree-q1-2-${qlist.QDid}`} value={2} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-8 h-8 mr-4" name={`question-${qlist.QDnumber}`} id={`neutral-q1-${qlist.QDid}`} value={3} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-8 h-8 mr-4" name={`question-${qlist.QDnumber}`} id={`agree-q1-2-${qlist.QDid}`} value={4} onChange={(event) => selectQ(event, qlist)} />


              <input type="radio" className="cursor-pointer w-10 h-10 mr-4" name={`question-${qlist.QDnumber}`} id={`agree-q1-3-${qlist.QDid}`} value={5} onChange={(event) => selectQ(event, qlist)} />

              <input type="radio" className="  cursor-pointer w-14 h-14  mr-4 " name={`question-${qlist.QDnumber}`} id={`disagree-q1-4-${qlist.QDid}`} value={6} onChange={(event) => selectQ(event, qlist)} />
